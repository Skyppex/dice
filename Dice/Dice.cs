...
public readonly record struct DiceRange(int Min, int Max, int Count, Func<float, string> Format) : IDice
{
    ...
    public int Roll()
    {
        ...
        return Enumerable.Range(0, Count)
            .Select(_ => Random.Shared.Next(min, max + 1)).Sum();
    }

    public override string ToString() => Min == 1 ? $"{Max}" : $"[{Min}, {Min}]";
}

public readonly record struct DiceValues(IEnumerable<int> SideValues, int Count, Func<float, string> Format) : IDice
{
